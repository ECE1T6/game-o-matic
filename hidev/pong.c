...
void draw_ball(struct ball *ball_ptr, bool array[(int)ARRAY_HEIGHT][(int)ARRAY_WIDTH], bool boolean){
	int i, j;
...
int main (void){
	...
	struct ball ball = {LEFT_MARGIN + (RIGHT_END - LEFT_MARGIN) / 2, TOP_MARGIN + (BOTTOM_END - TOP_MARGIN) / 2 -17, DISTANCE_PER_REFRESH * sqrt(0.5), - DISTANCE_PER_REFRESH * sqrt(0.5)};
	...
	while (1){
		...
		if (score_delay > 0){
			...
			if(ball_ptr->x_coord <= LEFT_IMPACT_X_COORD){ /*The ball has pierced the plane of left_paddle*/
				check_left_paddle_impact(ball_ptr, left_paddle_y_coord);
				...
				check_right_paddle_impact(ball_ptr, right_paddle_y_coord);
