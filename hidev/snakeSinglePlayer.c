...
static struct segment *initializeList(int INITIAL_X, int INITIAL_Y, int INITIAL_SIZE, bool **ledArray);
static void placeFood(int *foodX, int *foodY, struct segment *first, int HEIGHT, int WIDTH, int FIRST_TOP, int FIRST_LEFT, bool **ledArray);
static int moveHeadCoords(int moveDir, int visibleMoveDir, float *headX, float *headY, float moveDistance);
static void endGame(bool **ledArray, int foodY, int foodX, struct segment *first);
